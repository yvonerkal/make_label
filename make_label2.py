    ...
    <script>
        ...
        audio.addEventListener("timeupdate", function() {{
            ...
            if (now - lastUpdate > 200) {{  // 限制更新频率
                ...
                // 发送数据到Streamlit
                parent.window.stWebComponents = parent.window.stWebComponents || {{}};
                parent.window.stWebComponents["{key}"] = {{
...
def process_audio():
    ...
    if audio_state["current_index"] < len(unprocessed):
        ...
        with col_main:
            ...
            # 获取播放状态
            playback_position = audio_state.get("playback_position", 0.0)
            is_playing = audio_state.get("is_playing", False)
            
            # 波形图（单独一行）
            st.image(
                generate_waveform_image(segment_y, sr, playback_position if is_playing else None),
                caption="波形图", use_container_width=True
            )
            
            # 频谱图（单独一行）
            st.image(
                generate_spectrogram_image(segment_y, sr, playback_position if is_playing else None),
