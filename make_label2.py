...
from PIL import Image  # 移至顶部统一导入
...
def process_audio():
    ...
    if audio_state["current_index"] < len(unprocessed):
        ...
        # 布局：左侧音频信息，右侧标签（核心：确保标签在右侧）
        col_main, col_labels = st.columns([3, 1])  # 3:1比例，右侧为标签区域
...
if __name__ == "__main__":
    label_management_component()  # 加载标签（侧边栏）
    process_audio()  # 处理音频（右侧显示标签）
